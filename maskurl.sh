...
url_checker() {
    if [ ! "${1//:*}" = http ]; then
        if [ ! "${1//:*}" = https ]; then
            echo -e "\e[31m[!] Invalid URL. Please use http/https.\e[0m"
            exit 1
        fi
    fi
}

echo -e "\e[1;31m###Mask-Url###\e[0m \n"
echo -e "\e[1;31mBy\e[1;31m eZZ\e[0m \n\n"
echo -e "\e[1;31m### Phishing URL ###\e[0m \n"
echo -n "Import you website {use only http/https}: "
read phish
url_checker $phish
sleep 1
